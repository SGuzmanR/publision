...
import { FooterSocialLinks, Navlinks, SocialLinks } from "@/constants";
      ...
      <nav id="navbar" className="bg-white w-full flex flex-row justify-between items-center py-4 font-mulish px-4 rounded-lg">
        ...
        <div className="flex flex-row justify-center items-center gap-4">
          {SocialLinks.map((link) => (
            <Link key={link.name} href={link.href} className="navlinks translate-y-10 opacity-0"  target="_blank">
        ...
        <div className="max-[940px]:flex hidden navlinks -translate-x-10 opacity-0 cursor-pointer">
          <svg width="32px" height="32px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handleOpen}>
            ...
            <div className="flex gap-6">
              {FooterSocialLinks.map((link) => (
                <Link key={link.name} href={link.href} target="_blank" onClick={handleOpen} className="navLinks_mobile opacity-0">
                  <Image src={link.src} alt={link.name} width={30} height={30} className="hover:scale-105 transition-transform duration-500" />
                </Link>
              ))}
            </div>
